...
export function stretch_row_sparse(orig, new_size) {
...
export function enlarge(orig, new_width, new_height) {
    ...
    const sparse = Array.from(
        ...
        function(undef, i) {
            if (
                ...
                const stretched_row = stretch_row_sparse(orig[orig_coordinates], new_width)
                // prev_coordinates = next_coordinates
                orig_coordinates += 1
                next_coordinates = stretched_coordinates[orig_coordinates]
                return stretched_row
    ...
    const stretched = Array.from(
        ...
        (row, y) => Array.from(
            ...
            function(cell, x) {
                if (cell === null ) {
                    return evaluate_neighbours_sparse(sparse, x, y)
                } else {
                    return cell
                }
            }
