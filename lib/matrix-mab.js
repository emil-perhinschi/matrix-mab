...
export function add_matrices(first, second) {
    [first, second].forEach((m) => {
        if(!Array.isArray(m)) {
            throw new Error("must pass arrays to add_matrices")
        }

        if(typeof m[0][0] === "undefined") {
            throw new Error("not a 2d array: must pass 2d arrays to add_matrices")
        }
    })
...
export function random_matrix( width, height, callback = (z) => z) {
    return Array.from(
        Array(height),
        (v, y) => Array.from(
            Array(width),
...
export function sum_elements(matrix) {
    return matrix.reduce(
        (sum, row) => accumulator + row.reduce(
            (row_sum, cell) => row_sum + Number(cell)
        )
    )
}
